...
async function analyzeSilverMemberships() {
  try {
    ...
    if (oldSilverMemberships.length === 0) {
      console.log('\nNo Silver VIP memberships over 1 year old found.');
    } else {
      ...
      const ages = oldSilverMemberships.map(m => 
        Math.floor((Date.now() - new Date(m.createdOn).getTime()) / (1000 * 60 * 60 * 24 * 30))
      );
      console.log(`Oldest membership: ${Math.max(...ages)} months old`);
      console.log(`Newest membership: ${Math.min(...ages)} months old`);
      
      console.log('\n' + '=' .repeat(80));
      console.log('MEMBERSHIP IDS TO EXPIRE:');
      console.log('=' .repeat(80));
      console.log(oldSilverMemberships.map(m => m.id).join(', '));
    }
