/**
 * Mailgun Webhook Handler for ServiceTitan Estimate PDFs
 * 
 * Processes estimate emails with PDF attachments:
 * 1. Verifies Mailgun signature
 * 2. Extracts PDF attachment
 * 3. Parses PDF for customer data
 * 4. Matches customer in database
 * 5. Checks if $0 quote (skip if so)
 * 6. Triggers quote follow-up campaign
 */
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    console.log('[Estimate Webhook] Received email:', {
    ...
    console.log('[Estimate Webhook] Signature verified');
    ...
    console.log('[Estimate Webhook] Found PDF:', pdfFilename, `(${(pdfFile.size / 1024).toFixed(2)} KB)`);
    
    // TODO: Will implement PDF parsing, customer matching, and campaign triggering in next tasks
    // For now, mark as "pending" since parsing hasn't happened yet
    logEntry.status = 'pending'; // Will change to 'parsed' after actual parsing
    
    // Save log entry
    const [savedLog] = await db.insert(estimateProcessingLog).values(logEntry).returning();
    
    console.log('[Estimate Webhook] Logged successfully:', savedLog.id);
    ...
    try {
      ...
      console.error('[Estimate Webhook] Failed to log error:', dbError);
    }
