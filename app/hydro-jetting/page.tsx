...
interface HydroJettingPageProps {
  searchParams: Promise<{ [key: string]: string | string[] | undefined }>;
}

export async function generateMetadata(): Promise<Metadata> {
  return await getPageMetadata('/hydro-jetting', {
...
export default async function HydroJetting({ searchParams }: HydroJettingPageProps) {
  ...
  const canonical = 'https://www.plumbersthatcare.com/hydro-jetting-services';
  const serviceSchema = createServiceSchema(
    'Hydro Jetting Services',
    'Power through tough clogs in Austin with hydro jetting up to 4,000 PSI. Clear grease, tree roots & sewer buildup.',
    canonical
  );
  ...
  const breadcrumbSchema = createBreadcrumbListSchema([
    ...
    { name: "Hydro Jetting Services", url: canonical }
  ]);
