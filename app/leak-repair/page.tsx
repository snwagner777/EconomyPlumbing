...
/**
 * Leak Repair Services Page - Server Component with SEO
 * 
...
interface LeakRepairPageProps {
  searchParams: Promise<{ [key: string]: string | string[] | undefined }>;
}

export async function generateMetadata(): Promise<Metadata> {
  return await getPageMetadata('/leak-repair', {
    ...
    ogImage: 'https://www.plumbersthatcare.com/attached_assets/optimized/Leak_repair_service_work_cb3145cc.webp',
    ogType: 'website',
  });
}
...
export default async function LeakRepair({ searchParams }: LeakRepairPageProps) {
  ...
  const serviceSchema = createServiceSchema(
    ...
    'Fast leak detection with electronic equipment. Slab leak specialists, pipe & hidden leak repair.',
    canonical
  );
  ...
  const breadcrumbSchema = createBreadcrumbListSchema([
    ...
    { name: "Leak Repair Services", url: canonical }
  ]);
  ...
  return (
    <>
      ...
      <Script
        id="leak-repair-service-schema"
        type="application/ld+json"
        dangerouslySetInnerHTML={{ __html: JSON.stringify(serviceSchema) }}
      />
      <Script
        id="leak-repair-faq-schema"
        type="application/ld+json"
        dangerouslySetInnerHTML={{ __html: JSON.stringify(faqSchema) }}
      />
      <Script
        id="leak-repair-breadcrumb-schema"
        type="application/ld+json"
