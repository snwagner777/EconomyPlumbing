...
interface GasLeakDetectionPageProps {
  searchParams: Promise<{ [key: string]: string | string[] | undefined }>;
}

export async function generateMetadata(): Promise<Metadata> {
  return await getPageMetadata('/gas-leak-detection', {
...
export default async function GasLeakDetection({ searchParams }: GasLeakDetectionPageProps) {
  await searchParams;
  const { phoneConfig, marbleFallsPhoneConfig } = await getPhoneNumbers();

  const serviceSchema = createServiceSchema({
    name: "Gas Leak Detection & Repair Services",
    description: "Immediate gas leak detection & repair in Austin. Electronic equipment, licensed gas technicians, 24/7 emergency response for your safety.",
    areaServed: [
