...
interface GarbageDisposalRepairPageProps {
  searchParams: Promise<{ [key: string]: string | string[] | undefined }>;
}

export async function generateMetadata(): Promise<Metadata> {
  return await getPageMetadata('/garbage-disposal-repair', {
    title: 'Garbage Disposal Repair Austin TX | Same-Day Service',
    description: 'Fix jammed, leaking or broken garbage disposals in Austin. Expert repair & new disposal installation for all brands serviced. Same-day. Call (512) 368-9159.',
    ogType: 'website',
  });
}
...
export default async function GarbageDisposalRepair({ searchParams }: GarbageDisposalRepairPageProps) {
  await searchParams;
  const { phoneConfig, marbleFallsPhoneConfig } = await getPhoneNumbers();

  const serviceSchema = createServiceSchema({
    name: "Garbage Disposal Services",
    description: "Fix jammed, leaking or broken garbage disposals in Austin. Expert repair & new disposal installation for all brands serviced. Same-day service available.",
    areaServed: [
  ...
  const breadcrumbSchema = createBreadcrumbListSchema([
    ...
    { name: "Garbage Disposal Repair", item: "https://www.plumbersthatcare.com/garbage-disposal-repair" }
  ]);
