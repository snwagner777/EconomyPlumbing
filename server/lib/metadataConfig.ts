 ...
 * 
 * This provides the correct title, description, and canonical URL
 * for each route in the initial HTML (before JavaScript runs).
 * 
 * This approach matches your other successful 80%+ scoring projects.
 */
 ...
 */
export function getMetadataForPath(path: string): PageMetadata | null {
  // Normalize path (remove trailing slash, convert to lowercase)
  const normalizedPath = path.toLowerCase().replace(/\/$/, '') || '/';
  
  // Find exact match
  const metadata = pageMetadata.find(p => p.path === normalizedPath);
  
  return metadata || null;
}

/**
 * Generate default metadata for blog posts
 */
export function getDefaultBlogMetadata(slug: string): PageMetadata {
  // Format slug as title (capitalize words, replace hyphens)
  const title = slug
    .split('-')
  ...
  return {
    path: `/${slug}`,
    title: `${title} | Economy Plumbing Blog`,
    description: `Read about ${title.toLowerCase()} from Economy Plumbing. Expert plumbing tips and advice for Austin and Marble Falls homeowners.`,
  };
}
