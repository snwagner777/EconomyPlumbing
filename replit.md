...
- **Photo Management System Complete (October 11, 2025):** Fixed admin panel to show ALL 230 photos (189 CompanyCam + 41 Google Drive). Updated `getAllPhotos()` to combine both photo sources with unified display format. Added source badges ('google-drive' vs 'companycam') for identification. Fixed white SVG logo visibility by adding dark background containers in admin panel. Enhanced logo processor with proper SVG detection (checks file content for `<svg`/`<?xml`) and clear error messaging. Auto-converts supported raster formats (JPEG, WebP, GIF, BMP, TIFF) to PNG before AI processing.
