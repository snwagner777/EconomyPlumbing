...
- **Success Story Multi-Source Photo Bug Fix (October 11, 2025):** Fixed critical bug where success story creation failed with "One or both photos not found" error when selecting Google Drive photos. Updated `getPhotoById()` and `markPhotoAsUsed()` methods in storage layer to search BOTH companyCamPhotos and importedPhotos tables. These methods now properly handle photos from either CompanyCam or Google Drive sources by checking both tables and converting to unified format when needed. Success stories can now be created with any combination of photos from CompanyCam (189 photos) and Google Drive (41 photos).
