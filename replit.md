...
- **Universal Logo Processor with WebP Output (October 11, 2025):** Logo processor now accepts ALL image formats (SVG, PNG, JPEG, WebP, GIF, BMP, TIFF) and outputs optimized WebP files. Pipeline: (1) Converts any input format to PNG using Sharp for consistent processing, (2) Uses OpenAI Vision to analyze and remove background, (3) Creates white monochrome silhouette on transparent background, (4) Outputs as WebP (quality 90) for 25-35% smaller file sizes vs PNG. This eliminates previous SVG gradient/pattern issues by rasterizing all formats to pixels first.
