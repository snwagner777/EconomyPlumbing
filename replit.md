...
- **SSR for All Pages:** All new pages MUST have Server-Side Rendering (SSR) enabled for SEO. Static pages require adding config to `ssrPages` array in `server/lib/ssrRenderer.ts`. Dynamic pages (blog posts, etc.) are handled automatically via on-demand generation. No exceptions unless explicitly stated.
...
- **Dynamic Phone Number Tracking:** Database-driven system for automatic phone number insertion based on traffic source (Google Ads, Bing, Facebook, etc.), with cookie persistence across user sessions and an admin panel for management. Marketing phone numbers serve both paid traffic AND search engine crawlers (Googlebot sees Google Ads number, Bingbot sees Bing number).
- **Server-Side Rendering (SSR) for SEO:** Comprehensive event-driven SSR cache system for **all pages** including static pages AND dynamic blog posts. Crawlers (Googlebot, Bingbot, Ahrefsbot, etc.) receive pre-rendered HTML with proper meta tags, H1s, and structured data, eliminating SE Ranking issues (duplicate titles, missing H1s). Cache persists indefinitely until content changes, providing optimal performance (zero overhead after first request) while ensuring crawlers always see up-to-date content. **URL Normalization:** All trailing-slash URLs (e.g., `/about/`) automatically 301 redirect to non-trailing-slash versions (`/about`) to ensure crawlers always hit the SSR cache. This prevents duplicate content issues and ensures consistent H1/link visibility. **On-demand blog post generation:** Blog posts are SSR-rendered on first crawler visit, then cached. Reviews/blog posts shown on pages display curated content (top-rated reviews, latest posts) for consistency. Cache automatically invalidates when blog posts, tracking numbers, page metadata, or success stories are created/updated/deleted via API routes or background jobs. Individual blog post caches can be invalidated separately. **Static pages:** Home, services, contact, FAQ, success stories, membership benefits, and all service/area pages. **Adding SSR to new static pages:** Simply add page config to `ssrPages` array in `server/lib/ssrRenderer.ts` - includes title, description, h1, and content. Registration and cache invalidation happen automatically.
