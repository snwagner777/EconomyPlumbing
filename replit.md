...
- **AI-Powered Photo Quality System:**
  ...
  - **Multi-Source Photo Import:**
    - **CompanyCam via Zapier (Ongoing):** Webhook receives photos from CompanyCam jobs, downloads and saves locally to server in `attached_assets/imported_photos/{category}/`, analyzes with AI, stores quality photos automatically.
    - **Google Drive Bulk Import (One-Time):** Import historical photos from Google Drive folder (pagination supports 200+ images), downloads each to server, AI analyzes and categorizes, saves accepted photos to category-organized folders. Rejected photos remain in Google Drive but aren't saved to server.
    - **ServiceTitan Integration:** Can fetch photos from completed jobs in last 30 days.
  - **Local Storage:** All accepted photos downloaded and stored on server at `attached_assets/imported_photos/{category}/` organized by plumbing type (water_heater, faucet, drain, leak, toilet, gas, backflow, commercial, general).
  - **Database Tracking:** companyCamPhotos table with quality scores, AI descriptions, tags, local file paths, and usage tracking.
