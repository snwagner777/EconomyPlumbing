...
- **RSS Feeds:** Blog RSS feed with responsive WebP images served via `/api/blog/images/{base64}.jpg` endpoints (URLs end in .jpg for RSS reader compatibility). Success Stories RSS feed serves customer testimonials with dedicated `/api/success-stories/images/{base64}.jpg` endpoint for semantic clarity. Image conversion endpoints normalize object storage paths by stripping `/replit-objstore-{id}/public/` prefixes to match `searchPublicObject` requirements.
