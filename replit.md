...
- **Server-Side Rendering (SSR) for SEO:** Event-driven SSR cache system for critical landing pages. Crawlers (Googlebot, Bingbot, etc.) receive pre-rendered HTML with proper meta tags, H1s, and structured data, eliminating SE Ranking issues (duplicate titles, missing H1s). Cache persists indefinitely until content changes, providing optimal performance (zero overhead after first request) while ensuring crawlers always see up-to-date content. Cache automatically invalidates when blog posts, tracking numbers, page metadata, or success stories are created/updated/deleted via API routes or background jobs. Currently implemented for: `/schedule-appointment`, `/water-heater-services`, `/drain-cleaning`, `/leak-repair`, `/emergency-plumbing`. **Adding SSR to new pages:** Simply add page config to `ssrPages` array in `server/lib/ssrRenderer.ts` - includes title, description, h1, and content. Registration and cache invalidation happen automatically.
