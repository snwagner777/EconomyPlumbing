...
- **Social Media Image Alt Text**: Added server-side `og:image:alt` and `twitter:image:alt` tags to `ssrRenderer.ts` for complete social sharing metadata. Ensures Facebook, Twitter, and all crawlers see descriptive alt text without JavaScript execution. All 173+ pages now have proper image alt attributes for social media cards.
