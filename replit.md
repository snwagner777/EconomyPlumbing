...
- **Tracking Numbers Bulk Import & Table View (October 11, 2025):** Imported 19 tracking numbers from CSV with maximum detection rules (utm_source + referrerIncludes + platform-specific URL params). Updated default number to (512) 649-2811. Converted admin display from card layout to compact HTML table (5 columns: Channel, Phone Number, Detection Rules, Status, Actions) with icon-only buttons and hover effects. Total 25 tracking numbers: 1 default, 23 active channels (eLocal, Angie's List, Porch, SignPost, Bing, Birdeye, BBB, Cobiro, Apple Business, PlumbersInAustinTX.com, WaterHeatersInAustin.com, ResultCalls, NiceJob), 1 inactive (Halloween 2024). All detection rules combine multiple detection methods for maximum traffic capture.
- **Dynamic Phone Tracking UI Enhancement (October 11, 2025):** Replaced manual JSON textarea with user-friendly form fields for Detection Rules configuration. New interface features individual sections for URL Parameters (gclid, fbclid), UTM Sources (google, facebook), and Referrer Contains (google.com, facebook.com) with add/remove buttons and visual badges. JSON is auto-generated from fields upon save, eliminating syntax errors. Existing records are automatically parsed into individual fields when editing. Includes real-time JSON preview showing the generated detection rules.
- **Success Story Photo Preview Fix (October 11, 2025):** Fixed Create Success Story dialog showing "Before" and "After" text instead of actual photo images. Updated 5 locations in UnifiedAdminDashboard.tsx to use `photoUrl` property instead of incorrect `url` property for both CompanyCam and Google Drive photos. Fixes affect success story preview images, AI caption generation, blog post preview, and blog post AI generation.
