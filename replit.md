...
- **Top Customers Leaderboard:** Added "Most Loyal Customers" leaderboard to Refer-a-Friend page showing top 10 customers by completed service call count from ServiceTitan. Features performance-optimized backend with 1-hour in-memory cache (TTL), randomized sampling of 20 customers to prevent API rate throttling, stale-cache fallback on errors, medal styling for top 3 positions, anonymized names (First + Last initial), and "VIP Customer" badges. Displays alongside existing Top Referrers leaderboard for dual social proof and customer recognition.
